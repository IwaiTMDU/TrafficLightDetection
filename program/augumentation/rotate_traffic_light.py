	...
	def RotateImage(self, img):
		...
		for k in range(8):
			...
			if (_width > 0) and (_height > 0) :  
				M = cv2.getRotationMatrix2D((int(width/2), int(height/2)), math.degrees(angle), 1.0)
				M[0][2] += (_width - width)/2
				M[1][2] += (_height-height)/2
				img_rot = cv2.warpAffine(img, M, (int(_width), int(_height)))
				ok_flag = True
		...
		return img_rot, math.degrees(angle)
